...
The labeling interface we built is shown in the Figure ~\ref{fig:interface},  the interface is mainly divided into three parts in a vertical direction. The left part is the query tables to be labeled, and selecting the corresponding query table can display its corresponding information. The upper half in the middle part is the candidate tables corresponding to the query table, and the lower half displays the main information of the query table, including the meta information of the query table (such as all column names, cell values of corresponding column, etc.) and the query table itself, The upper part on the right of the interface is our labeling area, which is used to label whether two tables can be joined/unioned or not. The lower part is the main information of the candidate table, which also includes the meta information of the candidate table and the candidate table itself.
\begin{figure*}[h]
	\centering
